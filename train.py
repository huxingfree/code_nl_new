...
tf.app.flags.DEFINE_integer("code_vocab_size", 30000, "Natural language vocabulary size.")
...
def train():
    """Train a code->nl translation model using identifier-code data."""

    print("Preparing the data in %s" % FLAGS.data_dir)
    code_train, nl_train, code_dev, nl_dev, _, _ = data_utils.prepare_data(
        FLAGS.data_dir, FLAGS.code_vocab_size, FLAGS.nl_vocab_size, FLAGS.lang)
    ...
    with tf.Session(config=config) as sess:
        ...
        dev_set = read_data(code_dev, nl_dev)
        train_set = read_data(code_train, nl_train, FLAGS.max_train_data_size)
...
def test():
    ...
    with tf.Session() as sess:
        ...
        model.batch_size = 1
        counter = 0
        with tf.gfile.GFile(test_code_path, mode="r") as source_file:
...
def test_bleu():
    test_code_path = FLAGS.test_dir + '/test.ids' + str(FLAGS.code_vocab_size) + '.code'
    test_nl_path = FLAGS.test_dir + '/test.token.nl'
        ...
        with tf.gfile.GFile(test_code_path, mode="r") as source_file:
            with tf.gfile.GFile(test_nl_path, mode="r") as target_file:
                ...
                while source and target:
                    ...
                    result = {"score": score, "target": target, "output": o}
                    if score > 0.5:
                        print(target)
                        print(o)
                        print(score)
