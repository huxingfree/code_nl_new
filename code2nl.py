...
flags.DEFINE_float('learning_rate_decay_factor', 1.0,
                   'Learning rate decays by this much. Setting it to 1.0 will not affect the learning rate.')
...
flags.DEFINE_integer('batch_size', 128, 'Batch size to use during training.')
flags.DEFINE_integer('beam_size', 12, 'Max size of the beam used for decoding.')
flags.DEFINE_integer('num_samples_loss', 512,
                     'Number of samples to use in sampled softmax. Set to 0 to use regular loss.')
flags.DEFINE_integer('max_len', 120, 'Max size of the beam used for decoding.')
flags.DEFINE_integer('max_epochs', 100,
                     'Max number of epochs to use during training. The actual value will be (max_epochs-1) as it is 0-based.')
...
flags.DEFINE_string('attention_type', attention.GLOBAL,
                    'Which type of attention to use. One of local, global and hybrid.')
flags.DEFINE_integer('window_size', 10,
                     'Size of each size of the window to use when applying local attention. Not relevant to global attention')
flags.DEFINE_string('content_function', content_functions.VINYALS_KAISER,
                    'Type of content-based function to define the attention. One of vinyals_kayser, luong_general and luong_dot')
flags.DEFINE_boolean('use_lstm', True, 'Whether to use LSTM units. Default to False.')
flags.DEFINE_boolean('input_feeding', False,
                     'Whether to input the attention states as part of input to the decoder at each timestep. Default to False.')
flags.DEFINE_boolean('informed_decoder', True,
                     'Whether to use the previous word embedding info to the softmax. Default to False.')
flags.DEFINE_string('output_attention', content_functions.DECODER_TYPE_2,
                    'Whether to pay attention on the decoder outputs. Default to False.')
flags.DEFINE_integer('proj_size', 512, 'Size of words projection.')
flags.DEFINE_integer('hidden_size', 512, 'Size of each layer.')
flags.DEFINE_integer('num_layers', 1, 'Number of layers in each component of the model.')

flags.DEFINE_float('dropout', 0.2,
                   'Dropout rate. When the value is 0.0 dropout is turned off. Optimal should be 0.2 as indicated by Zaremba et al. (2014)')
...
flags.DEFINE_string('model_name',
                    'code2nl_globalAtten.ckpt',
                    'Model name')
flags.DEFINE_string('data_dir', '../data/java/1516', 'Data directory')
flags.DEFINE_string('train_dir',
                    '../data/java/1516/model/model_150To30/global_attention', 'Train directory')

flags.DEFINE_string('best_models_dir', '../data/java/1516/model/model_150To30/global_attention/best_models',
                    'Train directory')
flags.DEFINE_string('train_data', '../data/java/1516/train', 'Data for training.')
flags.DEFINE_string('valid_data', '../data/java/1516/train', 'Data for validation.')
flags.DEFINE_string('test_data', '../data/java/1516/test', 'Data for testing.')
flags.DEFINE_string('vocab_data', '', 'Training directory.')
flags.DEFINE_string('source_lang', 'code', 'Source language extension.')
flags.DEFINE_string('target_lang', 'nl', 'Target language extension.')
...
flags.DEFINE_boolean('log_tensorboard', False,
                     'Whether or not to use Tensorboard to log info about training. Default to False.')
